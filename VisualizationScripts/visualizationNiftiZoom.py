...
def visualize_and_save_image(image, output_folder, person_id):
    ...
    # Set the camera to a frontal view
    distance = 700  # Distance for zoom
    position = (0, distance, 290)  # Frontal view position (along y-axis)

    camera.SetPosition(position)
    camera.SetFocalPoint(0, 0, 0)
    camera.SetViewUp(0, 0, 1)  # Consistent view-up vector

    # Ensure the camera is set to see the volume
    renderer.ResetCamera()
    camera.Dolly(2)  # Zoom in change to 2.3 
    ...
    output_path = os.path.join(output_folder, f'person_{person_id}_view_frontal.png')
    writer.SetFileName(output_path)
    writer.SetInputConnection(window_to_image_filter.GetOutputPort())
    writer.Write()

def process_all_folders(base_directory, SUBFOLDERDIRECTORY, OUTPUTDIRNAME):
    output_folder = os.path.join(base_directory, OUTPUTDIRNAME)
    os.makedirs(output_folder, exist_ok=True)

    for i in range(150, 298):  # Adjust range as needed
...
if __name__ == "__main__":
    ...
    SUBFOLDERDIRECTORY = 'pyDefaceCleaned'  # Replace with the subfolder that has the niftis
    OUTPUTDIRNAME = 'pyDefaceImages'
    process_all_folders(BASEDIR, SUBFOLDERDIRECTORY, OUTPUTDIRNAME)
